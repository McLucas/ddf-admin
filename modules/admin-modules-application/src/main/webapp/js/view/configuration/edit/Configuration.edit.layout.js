...
define([
        ...
        'marionette',
        'underscore',
        'text!configurationEditLayoutTemplate',
        'text!configurationEditLayoutHeaderTemplate',
        'icanhaz',
        '/applications/js/view/configuration/edit/Configuration.edit.itemview.js',
        '/applications/js/view/configuration/edit/Configuration.edit.view.js'
        ...
        if(!ich.configurationEditLayoutTemplate) {
            ich.addTemplate('configurationEditLayoutTemplate', ConfigurationEditLayoutTemplate);
        }
        if(!ich.configurationEditLayoutHeaderTemplate) {
                        ...
                        if (type === STRING || type === CHARACTER || type === BYTE || (type >= DOUBLE && type <= BIGDECIMAL)) {
                            if(each.get('cardinality') !== 0) {
                                ...
                                if (propertyType === "newSave") {
                                    ...
                                    if(propertyType === "default") {
                                        view.collectionArray = new Backbone.Collection(valueArray);
                                        if(!view[each.get("id")]) {
                                            view.$(".data-section").append(ich.configurationEditLayoutHeaderTemplate(each.toJSON()));
                                            view.addRegion(each.get("id"), "#"+each.get("id"));
                                        }
                                        view[each.get("id")].show(new ConfigurationEditView({collection: view.collectionArray}));
                                    }
                                    else {
